...
| "How do updates work; do settings survive?" | Verified end to end and documented: check → download → "Restart to update" or apply on next exit (`WaitExitThenApplyUpdates`). Settings live outside Velopack's folder. A GitHub token setting was added because the repository is private and the releases API answers 404 anonymously. | See the update walk-through in `implementation.md`; the installed 0.1.0 was updated to 0.2.0 in place with its settings intact. |
