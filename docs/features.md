...
| Sentences and pauses | AssemblyAI is asked to wait for 1 s of silence before ending a sentence at a full stop (its default is 100 ms), so a pause to think no longer splits a sentence in two; releasing the hotkey still ends the dictation at once. The cleanup is also told to rejoin fragments cut at pauses (reliable at Medium and High; Light does it only sometimes, because it keeps edits minimal). |
